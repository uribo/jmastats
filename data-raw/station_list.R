...
if (!file.exists(here::here("data-raw/amedas_raw.rds"))) {
  ...
  df_amedas_master <-
    ...
    dplyr::mutate(dplyr::across(
      tidyselect::num_range("note", 1:2),
      .fns = list(
        ~ dplyr::if_else(.x %in% c("−", "\uff0d"), NA_character_, .x)
      ),
      .names = "{.col}"
    )) |>
    # csv上では「オホーツク」だが「網走・北見・紋別」
    dplyr::mutate(
      area = dplyr::recode(area, `オホーツク` = "網走・北見・紋別")
    ) |>
  ...
  df_prec_no <-
    xml2::read_html(
      "https://www.data.jma.go.jp/stats/etrn/select/prefecture00.php?prec_no=&block_no=&year=&month=&day=&view="
    ) |>
...
ne_jpn <-
  ...
  select(iso_3166_2) |>
  mutate(
    pref_code = stringr::str_remove(iso_3166_2, "JP-"),
    geometry,
    .keep = "none"
  )
...
stations <-
  stations |>
  mutate(
    pref_code = recode(station_no, !!!prefecture_code, .default = pref_code)
  )
...
tide_station <-
  ...
  purrr::map(
    function(.x) {
      Sys.sleep(7)
      polite::bow(stringr::str_glue(
        "https://www.data.jma.go.jp/gmd/kaiyou/db/tide/genbo/station.php?year={.x}"
      )) |>
        polite::scrape() |>
...
tide_station <-
  tide_station |>
  dplyr::filter(
    stringr::str_detect(`地点番号`, "\uff0a", negate = TRUE),
    `地点番号` != "地点番号"
  ) |>
  ...
  mutate(across(
    ...
    .fns = list(\(x) stringr::str_replace_all(x, c("\u309c" = "\u00b0"))),
    .names = "{.col}"
  )) |>
  mutate(
...
earthquake_station <-
  ...
  mutate(
    longitude = purrr::pmap_chr(
      .,
      ~ as.character(kuniezu::parse_lon_dohunbyo(paste0(
        "東経",
        ..7,
        "度",
        ..8,
    ...
    latitude = purrr::pmap_chr(
      .,
      ~ as.character(kuniezu::parse_lat_dohunbyo(paste0(
        "北緯",
        ..5,
        "度",
        ..6,
