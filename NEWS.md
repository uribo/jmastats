...
* Various datasets handled by the package have been updated to the latest version in September 2026. Stations that are no longer in operation have been removed from `stations`, so their `block_no` can no longer be passed to `jma_collect()`.
