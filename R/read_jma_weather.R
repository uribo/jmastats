...
read_jma_weather <- function(path) {
  ...
  var_names_combination <-
    paste(var_st_names, var_names, sep = "_")
  d <-
    readr::read_csv(
      path,
      skip = 5,
      locale = readr::locale(encoding = "cp932"),
      col_names = c("date", var_names_combination),
      col_types = paste0(
        "c",
        paste(
          rep(
            "c",
            length(var_names_combination)
          ),
          collapse = ""
        )
      )
...
read_jma_single_row <- function(path, row, enc_convert = TRUE) {
  ...
  readr::read_lines(path, skip = row - 1, n_max = 1, locale = locale) |>
...
station_head_info <- function(path) {
  ...
  cat(
    crayon::black(paste0(
      "Selected station",
      dplyr::if_else(length(target_st) == 1L, "", "s"),
      ":"
    )),
    crayon::cyan(
