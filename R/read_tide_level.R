...
request_tide_level_url <- function(.year, .month, .stn) {
  ...
  month <-
    sprintf("%02d", .month)
  month <-
    rlang::arg_match(month, sprintf("%02d", seq.int(12)))
  ...
  stringr::str_glue(
    "{jma_site}/gmd/kaiyou/data/db/tide/genbo/{year}/{year}{month}/hry{year}{month}{stn}.txt"
  ) # nolint
}
...
parse_tide_file <- function(data) {
  ...
  seq.int(nrow(data)) |>
    ...
    dplyr::mutate(
      dplyr::across(c(tidyselect::contains("tide_hm_obs")), function(x) {
        dplyr::na_if(x, "99:99")
      })
    ) |>
    dplyr::mutate(
      dplyr::across(
        c(tidyselect::contains("tide_tidal_level_cm_obs")),
...
parse_hry <- function(d) {
  purrr::map2_chr(
    ...
    as.data.frame(stringsAsFactors = FALSE) |>
    dplyr::mutate(
      dplyr::across(tidyselect::everything(), readr::parse_number)
    ) |>
    tibble::as_tibble()
}
...
parse_ymd <- function(d, type) {
  type <-
    rlang::arg_match(type, c("year", "month", "day"))
  index_list <-
    list(x = 73, y = 74)
  index_list <-
    switch(
      ...
      "year" = index_list,
      "month" = index_list |>
        purrr::list_modify(x = index_list$x + 2, y = index_list$y + 2),
      "day" = index_list |>
        purrr::list_modify(x = index_list$x + 4, y = index_list$y + 4)
    )
...
parse_hm <- function(d, time) {
  time <-
    rlang::arg_match(time, c("low_tide", "high_tide"))
  index_list <-
    switch(
      time,
      low_tide = list(
  ...
  d <-
    purrr::map2(
      ...
      ~ d[seq.int(.x, .y)] |>
        ...
        dplyr::mutate(
          across(tidyselect::everything(), function(x) {
            stringr::str_replace(x, pattern = "[[:blank:]]", replacement = "0")
          })
        ) |>
        dplyr::mutate(
          hm = paste(
            paste0(h1, h2, collapse = ""),
    ...
    purrr::list_cbind()
  d |>
    purrr::set_names(paste0(time, "_hm", "_obs", seq.int(length(d))))
}
  ...
  d <-
    purrr::map2(
      ...
      ~ d[seq.int(.x, .y)] |>
        ...
        dplyr::mutate(
          tidal_level_cm = paste0(obs1, obs2, obs3, collapse = ""),
          .keep = "none"
        )
    ) |>
    purrr::list_cbind()
  d |>
    purrr::set_names(paste0(time, "_level", "_obs", seq.int(length(d))))
}
