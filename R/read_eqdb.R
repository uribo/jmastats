...
read_eqdb_csv <- function(path, show_metadata = TRUE) {
  if (show_metadata == TRUE) {
    meta <-
      list(
        item = list(
          c(22320, 38663, 12398, 27010, 35201),
        ...
        vars = readr::read_csv(path, n_max = 1, col_types = "Dtccccdc")
      )
    cli::cli_inform(
      c(
        intToUtf8(meta$item[[1]]),
        "x" = stringr::str_glue(
          "{intToUtf8(meta$item[[2]])}: {meta$vars[[1]]} {meta$vars[[2]]}"
        ),
        "x" = stringr::str_glue(
          "{intToUtf8(meta$item[[3]])}: {meta$vars[[3]]}"
        ),
        "*" = stringr::str_glue(
          "{intToUtf8(meta$item[[4]])}: {meta$vars[[4]]}"
        ),
        "*" = stringr::str_glue(
          "{intToUtf8(meta$item[[5]])}: {meta$vars[[5]]}"
        ),
        "!" = stringr::str_glue(
          "{intToUtf8(meta$item[[6]])}: {meta$vars[[6]]}"
        ),
        "!" = stringr::str_glue(
          "{intToUtf8(meta$item[[7]])}: {meta$vars[[7]]}"
        ),
  ...
  record <-
    readr::read_csv(path, skip = 2, col_types = "ccc")
  record |>
    ...
    dplyr::mutate(
      !!rlang::sym(intToUtf8(c(38663, 24230))) := stringr::str_remove(
        !!rlang::sym(intToUtf8(c(38663, 24230))),
        "\u9707\u5ea6"
      ) |>
        dplyr::case_match(
          intToUtf8(65297) ~ "1",
